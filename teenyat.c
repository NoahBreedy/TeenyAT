 ...
 */
 ...
 teenyat* debugger_data;
 bool debugger_instance;
...
int debug_instance_check(teenyat *t){
    #if defined(_WIN64) || defined(_WIN32) 
        ...
        if (!hMapFile) {
            ...
            debugger_data = (teenyat*)MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, SHM_SIZE);
            
            if (!debugger_data) {
                printf("Error accessing shared memory pointer");
                CloseHandle(hMapFile);
                return 0;
            }

            memcpy(debugger_data,t,sizeof(teenyat));
 ...
 bool tny_init_from_file(teenyat *t, FILE *bin_file,
     ...
     /* CHECK IF TEENYAT IS A DEBUGER PROCESS */
     debugger_instance = debug_instance_check(t);
     if(!debugger_instance) printf("Not a debugger instance\n");
 ...
 void tny_clock(teenyat *t) {
     ...
     */
     ...
     if(debugger_instance) memcpy(debugger_data,t,sizeof(teenyat));
