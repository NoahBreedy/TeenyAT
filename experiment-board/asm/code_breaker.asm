...
;-------------------------------------------
    !no_input
    jmp !buffered_timer  ; This is out scuffed timer code.....
...
!resourceful_wait
    ...
    !resourceful_wait_loop
        ...
        inc rA
        cmp rA, 0xFFE ; BUG IN ASSEMBLER origional inst (cmp rA, 0xFFFF)
        jge !check_values_resorceful_wait
        jmp !resourceful_wait_loop
    !check_values_resorceful_wait
          lod rA, [ RESORCEFUL_WAIT_TIMER ]
          cmp rA, 0x8  ; BUG IN ASSEMBLER origional inst (cmp rA, 0xFFFF)
